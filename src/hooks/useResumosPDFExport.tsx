  ...
  const exportarResumo = async (subtema: SubtemaResumo, area: string) => {
    ...
    try {
      ...
      try {
        ...
        const fileName = `resumo-${subtema.subtema?.replace(/[^a-zA-Z0-9]/g, '_')}-${Date.now()}.pdf`;
        ...
        console.error('Erro no storage, fazendo download local:', storageError);
        // Fallback: download local
        doc.save(`resumo-${subtema.subtema?.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`);
      }
