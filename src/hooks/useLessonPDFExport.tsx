...
export const useLessonPDFExport = () => {
  ...
  const exportLessonSummary = async (content: LessonSummary, lesson: LessonData) => {
    ...
    try {
      ...
      // Upload para Supabase e abrir no navegador
      ...
      try {
        ...
        if (userId) {
          ...
          const filePath = `resumos/${userId}/${timestamp}-${nomeArquivo}`;
          ...
          if (!uploadError) {
            ...
            if (urlData.publicUrl) {
              // Abrir PDF no navegador fora do app
      ...
      toast({
        title: "PDF exportado!",
        description: "Resumo da aula exportado com sucesso.",
      });
