...
export const AIDocumentAnalyzer = ({ onBack }: AIDocumentAnalyzerProps) => {
  ...
  const exportToPDF = async () => {
    try {
      ...
      const { data, error } = await supabase.functions.invoke('export-conversation-pdf', {
        body: {
      ...
      if (error) {
        ...
        throw new Error(error.message || 'Erro ao exportar conversa');
      }

      if (!data.success) {
        throw new Error(data.error || 'Erro ao processar exportação');
      }
      ...
      toast({
        ...
        description: "A conversa foi salva temporariamente. O link expira em 24 horas.",
        action: (
          <Button 
            variant="outline" 
            size="sm" 
